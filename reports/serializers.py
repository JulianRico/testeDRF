...
class ReportSerializer (serializers.ModelSerializer):
   ...
   # status  = serializers.SlugRelatedField(slug_field='name', queryset=Report.objects.all())
    companie = serializers.SlugRelatedField(slug_field='name', queryset=Companie.objects.all())
    
    class Meta:
        model = Report
        fields = ('id', 'status_display', 'status','questionsmtto','questionviews', 'questionsdeterioration', 'user','companie','create_at', )
